...
# 读取数据 (替换为你的文件路径)
df <- read.csv("/Users/xifei/Desktop/Marketing Analysis/Assign1/SmartFresh Retail.csv") # 请替换为正确的文件路径
...
df_filtered <- df %>% filter(Marital_Status %in% c("Single", "Divorced"))
...
ggplot(df_filtered, aes(x = Spend_Wine, fill = Marital_Status)) +
  ...
  theme_minimal() +
  labs(title = "Wine Spending Distribution: Single vs. Divorced",
...
ggplot(df_filtered, aes(x = Log_Spend_Wine, fill = Marital_Status)) +
  ...
  geom_density(alpha = 0.7) +
  theme_minimal() +
  labs(title = "Log-Transformed Wine Spending Distribution: Single vs. Divorced",
       x = "Log(Wine Spending + 1)", y = "Density")

# **检查正态性（Shapiro-Wilk 检验）**
shapiro.test(df_filtered$Log_Spend_Wine[df_filtered$Marital_Status == "Single"])
shapiro.test(df_filtered$Log_Spend_Wine[df_filtered$Marital_Status == "Divorced"])
